...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
 ...
 <!--xsl:import href="Q:/dev/qv/qv_editor/src/webapp/dist/skins/common/qti.xsl"/>
 <xsl:import href="Q:/dev/qv/qv_editor/src/webapp/dist/skins/generic/generic.xsl"/-->	
 ...
 <xsl:template name="get_section_path">
	 javascript:gotoSection('<xsl:value-of select="@num"/>');
 </xsl:template>	
  ...
  <xsl:template name="go_section">
    ...
    <xsl:variable name="url">if (!isSubmitted()) {gotoSection('<xsl:value-of select="normalize-space($section_to_go)"/>');}</xsl:variable>
 ...
 <xsl:template name="footer">
	<script type="text/javascript">
		writeCorrectQVApplet('<xsl:value-of select="//assessment/@ident"/>', '<xsl:value-of select="//section[position()=$section_number]/@ident"/>', '<xsl:value-of select="$section_number"/>');
		window.onload=init;
	</script>
 </xsl:template>
