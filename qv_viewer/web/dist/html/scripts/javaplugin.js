...
//   else{
	//alert("else");	
     ...
     document.write(' ARCHIVE="'+archive+'"');
     document.writeln(' WIDTH="' +nsw+ '" HEIGHT="' +nsh+ '" params="aaaa">');
     document.writeln('<PARAM NAME="type" VALUE="application/x-java-applet;version=1.3">');
     document.writeln('<PARAM NAME="scriptable" VALUE="'+scriptable+'">');
     writeParams2(params, true);
     document.writeln('</APPLET>');
...
function writeJavaAppletString(code, jarBase, archive, params, width, height, rWidth, rHeight, type, name, scriptable){
   s = "";
   var w=width.toString();
   var h=height.toString();
   var nsw=w;
   ...
   if(rHeight!=null) h=rHeight.toString();
     ...
     s = s+('<APPLET NAME="'+name+'" CODE="'+code+'" CODEBASE="' +jarBase+ '"');
     s = s+(' ARCHIVE="'+archive+'"');
     s = s+(' WIDTH="' +nsw+ '" HEIGHT="' +nsh+ '">');
     s = s+('<PARAM NAME="type" VALUE="application/x-java-applet;version=1.3">');
     s = s+('<PARAM NAME="scriptable" VALUE="'+scriptable+'">');
     s = s+writeParams2String(params, true);
     s = s+('</APPLET>');
     return s;
     //document.write (s);
}
...
function writeParams2String(params, p){
	var r = 0;
	s2 = "";
	var separator = ';';
	if (params.indexOf('$$')>0) separator='$$';
	j=params.indexOf('=');
	while (j>0){
		r=params.indexOf(separator);
		if (r<0) r=params.length;
		param=params.substring(0,r);
		paramName=param.substring(0,j);
		paramValue=param.substring(j+1);
		if(p) s2 = s2+('<PARAM NAME="'+paramName+'" VALUE="'+paramValue+'">');
		else s2 = s2+(' ' +paramName+ '="' +paramValue+ '" ');
		params=params.substring(r+separator.length);
		j=params.indexOf('=');
	}
	return s2;
}
...
function writeDragDropApplet(params, width, height, name){
	writeJavaApplet('QVDragDrop.class', '../../qv/appl', 'QVDragDrop.jar',params, width, height, null, null,'qvApplet', name, true);
}

function writeHotEqnApplet(params, width, height, name){
	writeJavaApplet('dHotEqn.class', '../../qv/appl', 'HotEqn.jar',params, width, height, null, null,'qvApplet', name, true);
}
