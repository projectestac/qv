...
String sWidth = "";
String sHeight = "";
if (o!=null && sMaterialAction!=null && sMaterialAction.equalsIgnoreCase(IQVListControlBean.A_SET_LIST_OBJECT) && !"cancel_material".equals(sAction)){
	sMatContentType = o.getMatContentType();
	sMatType = o.getMattype();
	sText = o.getText();
	sURI = o.getURI();
	...
	if (o instanceof edu.xtec.qv.qti.Matjclic || o instanceof edu.xtec.qv.qti.Matlatex){
		sWidth = String.valueOf(o.getWidth());
		sHeight = String.valueOf(o.getHeight());
	}
}
//System.out.println("ident="+sIdent+"  text="+sText+" action="+sAction);
...
vContentTypes.addElement(edu.xtec.qv.qti.QTISuperMat.MATFLASH_CONTENT_TYPE);
...
vContentTypes.addElement(edu.xtec.qv.qti.QTISuperMat.MATJCLIC_CONTENT_TYPE);
vContentTypes.addElement(edu.xtec.qv.qti.QTISuperMat.MATAPPLICATION_CONTENT_TYPE);
//vContentTypes.addElement(edu.xtec.qv.qti.QTISuperMat.MATLATEX_CONTENT_TYPE);
...
<TABLE class='layer-box' border='0' cellpadding='5' cellspacing='5' width='600px' height='100%' style='height:100%'>
<TR>	
...
<% if (QTISuperMat.isMatHTML(sMatContentType)){ %>  	
	<TR>	
		<TD class="layer-link" colspan="2" align="right" width="350">
			<A href="javascript:<%=sForm%>.hide_layers.value='true';<%=sForm%>.p_list_name.value='<%=sListName%>';<%=sForm%>.<%=sListName%>.value='<%=sIdent%>';<%=sForm%>.p_list_type.value='<%=sType%>';enviar('<%=sMaterialAction%>', <%=sForm%>);" title="<%=qvb.getMsg("material.action.ok.tip")%>" class="layer-link"><span style="text-decoration:none">&nbsp;</span><%=qvb.getMsg("material.action.ok")%></A>
			&nbsp;|&nbsp;
			<A href="javascript:<%=sForm%>.p_material_jclic_uri.value='';<%=sForm%>.p_content_type.value='';hide_material_layer();enviar('cancel_material', <%=sForm%>);" title="<%=qvb.getMsg("material.action.cancel.tip")%>" class="layer-link"><span style="text-decoration:none">&nbsp;</span><%=qvb.getMsg("material.action.cancel")%></A>
		</TD>
	</TR>
<%} %>	
  	<TR>
  		<TD colspan="2" style='height:250px' valign="top">
			...
			<jsp:include page="matflash.jsp" flush="true">
				<jsp:param name="p_material_uri" value="<%=sURI%>" />
				<jsp:param name="p_material_width" value="<%=sWidth%>" />
				<jsp:param name="p_material_height" value="<%=sHeight%>" />
				<jsp:param name="display" value="<%=String.valueOf(o!=null && QTISuperMat.isMatflash(sMatContentType, sMatType))%>" />
			</jsp:include>
			...
			<jsp:include page="matjclic.jsp" flush="true">
				<jsp:param name="p_material_jclic_uri" value="<%=sURI%>" />
				<jsp:param name="p_material_jclic_width" value="<%=sWidth%>" />
				<jsp:param name="p_material_jclic_height" value="<%=sHeight%>" />
				<jsp:param name="display" value="<%=String.valueOf(o!=null && QTISuperMat.isMatjclic(sMatContentType))%>" />
			</jsp:include>
			
			<jsp:include page="matapplication.jsp" flush="true">
				<jsp:param name="p_material_application_uri" value="<%=sURI%>" />
				<jsp:param name="display" value="<%=String.valueOf(o!=null && QTISuperMat.isMatapplication(sMatContentType))%>" />
			</jsp:include>

			<jsp:include page="matlatex.jsp" flush="true">
				<jsp:param name="p_material_latex_equation" value="<%=sText%>" />
				<jsp:param name="p_material_latex_width" value="<%=sWidth%>" />
				<jsp:param name="p_material_latex_height" value="<%=sHeight%>" />
				<jsp:param name="display" value="<%=String.valueOf(o!=null && QTISuperMat.isMatlatext(sMatContentType))%>" />
			</jsp:include>
  		</TD>
  	</TR>
  	</TABLE>
