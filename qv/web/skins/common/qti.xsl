 ...
 <xsl:variable name="common_images_path">skins/common/image</xsl:variable>
  ...
  <xsl:template match="*[local-name()='render_draw']">
	  ...
	  <xsl:variable name="applet">
			writeJavaApplet('edu.xtec.qv.applet.QVDrawer.class', './appl',
...
<!--    <xsl:copy-of select="."/>-->
  ...
  <xsl:template match="*[local-name()='matjclic']">
	...
	<script language="JavaScript" SRC="http://clic.xtec.cat/dist/jclic/jclicplugin.js" type="text/javascript"></script>
	  <script language="JavaScript"> 
		  ...
		  setLanguage('ca'); 
		  writePlugin('<xsl:value-of select="$jclic_uri"/>', '<xsl:value-of select="@width"/>', '<xsl:value-of select="@height"/>'); 
	  </script>
  ...
  <xsl:template match="*[local-name()='matlatex']">
    ...
    <script language="JavaScript"> 
		writeJavaApplet('dHotEqn.class', './appl', 'HotEqn.jar','<xsl:value-of select="$params"/>', '<xsl:value-of select="@width"/>', '<xsl:value-of select="@height"/>', null, null,'qvApplet', 'hotEqnApplet', true);
	</script>
  </xsl:template>	
        ...
        <xsl:variable name="new_src">
          <xsl:choose>
            ...
            <xsl:when test="starts-with($src, '/')">
              http://localhost:8080<xsl:value-of select="$src"/>
            </xsl:when>
            <xsl:otherwise>
  ...
  <xsl:template name="putMataudio">
    ...
    <SPAN>
	  ...
	  <script type="text/javascript">
		writeAudio('<xsl:value-of select="generate-id()"/>', '<xsl:value-of select="$file"/>', 'audio/mpeg',200,18);
	  </script>
      <IMG src="web/imatges/blanc.gif" width="5"/>
    </SPAN>
  </xsl:template>
...
<!-- #################### COMMONS - SHOW ################# -->
 ...
 <xsl:template name="show_save_button">
  ...
  <xsl:template name="show_feedback">
   <xsl:variable name="show">
    <xsl:choose>
       <xsl:when test="not (normalize-space(itemcontrol/@feedbackswitch)='No') and ($es_correccio='true')">true</xsl:when>
       <xsl:otherwise>false</xsl:otherwise>
     </xsl:choose>
   </xsl:variable>
   <xsl:value-of select="normalize-space($show)"/>  
...
<!-- #################### COMMONS - ACTIONS ################# -->
  ...
  <xsl:template name="go_section">
    ...
    <xsl:variable name="url">if (!isSubmitted()) {document.forms[0].next.value=<xsl:value-of select="$section_number_to_go"/>;
