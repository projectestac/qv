...
function openWindow(url, popW, popH){
    ...
    window.open(url,'qv','toolbar=yes,directories=false,scrollbars=yes,resizable=yes,width=' + popW + ',height=' + popH + ',top=' + topPos + ',left=' + leftPos+ ',screenY=' + topPos + ',screenX=' +
leftPos);
}
...
function generate_responses(test, bCheck){
  ...
  for(i=0; i<test.elements.length; i++){
	...
	// v=obj.value;
	//alert("name:"+obj.name+"="+obj.value+" id="+obj.id+" obj.checked?"+obj.checked);
...
function doSubmit2(){
	return doSubmit2(true);
}

function doSubmit2(bCheck){
  var bResult=false;
...
function doSubmit2x(){
	return doSubmit2x(true);
}

function doSubmit2x(bCheck){
  var bResult=false;
  var s=generate_responses(document.forms[0], bCheck);
  if(s!=null && s.length>0){
    bResult=doSubmit3();
