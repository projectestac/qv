...
public class QTIResponseParser extends Object {
    ...
    private Vector getSections(java.util.Iterator itSections){
        Vector vSection=new Vector();
        if (itSections!=null){
            int iNumFull=1;
            while(itSections.hasNext()){
                org.jdom.Element eSection=(org.jdom.Element)itSections.next();
                String sId=eSection.getAttributeValue("ident");
                Object oTitle=eSection.getAttributeValue("title");
                String sTitle=(oTitle!=null)?oTitle.toString():"Full "+iNumFull;
                FullAssignacio oSection = new FullAssignacio(iNumFull, sId, sTitle);
                vSection.add(oSection);
                iNumFull++;
            }
        }
        return vSection;
                ...
                if (sCurrentResponse!=null && !sCurrentResponse.trim().equals("")){
                    ...
                    if (st2.hasMoreTokens()){
                        ...
                        if (st2.hasMoreTokens()){
                            ...
                            if (sIdValue.indexOf(':')>0){ //Resposta de drag_drop...
